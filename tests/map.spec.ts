...
test('renders the world choropleth and key controls', async ({ page }) => {
  ...
  await expect(page.locator('.legend')).toContainText('Modeled year')
  await expect(page.locator('.legend')).toContainText('Latest before to year')
  await expect(page.locator('.coverage-strip')).toContainText('212 with 2024: 67 reported / 145 modeled')
...
test('reports reverse, same-year, and missing selected-year states', async ({ page }) => {
  ...
  await expect(page.locator('.detail-panel')).toContainText('Estimated same year from 2024 to 2024')
  await expect(page.locator('.detail-panel')).toContainText('Modeled 2024 estimate')
...
test('search selects a country and opens data notes', async ({ page }) => {
  ...
  await expect(page.locator('.source-panel')).toContainText('145 additional')
  await expect(page.locator('.source-panel')).toContainText('Modeled 2024 estimates')
...
test('search supports keyboard navigation and no-results feedback', async ({ page }) => {
  ...
  await expect(page.locator('.detail-panel')).toContainText('Estimated decrease from 2019 to 2024')
  await expect(compareStat).toContainText('Modeled 2024 estimate')
